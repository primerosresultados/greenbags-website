...
- **Catálogo + carrito + checkout** (productos simples y variables, categorías jerárquicas, atributos, tiers de precio por volumen, cupones, pasarelas Webpay/Flow/MercadoPago + transferencia y COD).
- **Módulo de cotizaciones (RFQ)** para sitios B2B sin precios públicos. Activable con un setting. Ver [docs/QUOTES_MODULE.md](docs/QUOTES_MODULE.md).
- **Banners hero** editables por posición.
- **Mini-CMS de páginas** (slug + título + body HTML + SEO), accesible en `/slug`.
- **Media library** con jerarquía de carpetas, WebP + srcset.
- **Información del negocio + sucursales** + JSON-LD LocalBusiness automático.
- **Mailing** con backend `mail()` o **Resend** (deliverability).
- **Notificación por email** al admin cada vez que entra un lead/cotización, y auto-respuesta opcional al cliente.
