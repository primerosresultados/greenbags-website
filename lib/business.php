...
function publishedPagesMenu(): array {
    try {
        // Excluye páginas marcadas con exclude_from_menu (la "Gracias" del
        // post-lead, legales, etc.). Fallback para instalaciones sin migración.
        return getDB()->query(
            'SELECT slug, title FROM pages
             WHERE is_published = 1 AND exclude_from_menu = 0
             ORDER BY title ASC'
        )->fetchAll();
    } catch (Throwable $e) {
        try {
